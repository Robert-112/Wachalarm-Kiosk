...
cp -v "$DIR/home/wachalarm_einstellungen.txt" "$MOUNTED_BOOT_VOLUME/wachalarm_einstellungen.txt"
...
working "Startvorgang anpassen" # https://scribles.net/customizing-boot-up-screen-on-raspberry-pi/
...
echo "* SD-Karte entfernen & in Pi einsetzen"
echo "* Raspberry Pi mit Netzwerk verbinden"
echo "* Pi booten / starten"
echo "* es folgt eine Password-Abfrage fuer den SSH-Zugriff auf den Raspberry"
echo "IP-Adresse eingeben:"
read IP

working "SSH-Key des Servers wird auf Client hinterlegt"
echo -e "(Hinweis 1: zuvor \"sudo ssh-keygen -t rsa\" auf dem Server ausfuehren)"
echo -e "(Hinweis 2: Password \"wachalarm\")"
...
working "Konsolen-Logins anpassen" # this is to avoid a brief flash of the console login before X comes up
...
working "Inhalt des home-Ordners uebertragen"
...
working "Skripte ausfuehrbar machen"
ssh "chmod +x *.sh && chmod +x .xsession"

working "Bild beim Startvorgang hinterlegen"
...
working "Crontab installieren"
ssh "crontab /home/pi/crontab.example"

working "VNC-Dienst einrichten (Kennwort \"wachalarm\")"
...
echo "Warten auf Neustart des Raspberry-Pi"
